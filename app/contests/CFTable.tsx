...
import { cfTotalProblems } from "./cfTotalData";
          ...
          <table className="table table-bordered table-hover rounded">
            ...
            <tbody>
              {contestList.map((contest) => {
                ...
                let isAttempted = userData[id]
                  ? !!Object.keys(userData[id]).length
                    ? "1"
                    : "0"
                  : "0";
                ...
                if (
                  ...
                  const date = new Date(contest.startTimeSeconds * 1000)
                    ...
                    .slice(1)
                    .join(" ");
                  const total = cfTotalProblems[id] || [];
                  const solved = userData[id]
                    ? Object.keys(userData[id]).sort()
                    : [];
                  const unSolved = total.filter(
                    (item) => !solved.includes(item)
                  );
                  ...
                  unSolvedString =
                    contest.phase === "BEFORE" ? "Upcoming" : unSolvedString;
                  ...
                  return (
                    <tr key={id} className="align-middle">
                      ...
                      <td
                        className={`${
                          solved.length ? "bg-solved text-white" : ""
                        }`}
                      >
                        {solved.length
                          ? solved.map((question) => (
